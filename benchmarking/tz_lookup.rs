...
fn binary_search(sorted: &[(&str, u16)], id: &str) -> Option<u16> {
    sorted.binary_search_by_key(&id, |&(name, _)| name).ok().map(|pos| sorted[pos].1)
}
...
fn bench_cal_write_path(c: &mut Criterion) {
    let mut cal_sorted: Vec<(&str, u8)> =
        CAL_IDS.iter().enumerate().map(|(i, &id)| (id, u8::try_from(i).unwrap())).collect();
...
fn bench_tz_hashmap_vs_bsearch(c: &mut Criterion) {
    ...
    for (label, tz) in [
        ...
        group.bench_with_input(criterion::BenchmarkId::new("HashMap", label), tz, |b, tz| {
            b.iter(|| map.get(black_box(tz)))
        });
        group.bench_with_input(criterion::BenchmarkId::new("binary_search", label), tz, |b, tz| {
            b.iter(|| binary_search(&sorted, black_box(tz)))
        });
    }
    group.finish();
}
